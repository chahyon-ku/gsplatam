...
from SplaTAM.utils.slam_helpers import get_depth_and_silhouette
from SplaTAM.utils.slam_external import build_rotation
...
def tk_thread():
    ...
    root.geometry("+100+100")  # top-left corner is at screen coords (100,100)
    root.attributes('-topmost', True)  # Attempt to keep it on top
    root.mainloop()

def translate_camera(vis, dx=0, dy=0, dz=0):
    # Get current view parameters
    view_ctl = vis.get_view_control()
    params = view_ctl.convert_to_pinhole_camera_parameters()
    # Extract the current extrinsic matrix
    extrinsic = params.extrinsic.copy()
    # Create a translation vector
    translation = np.array([dx, dy, dz])
    # Apply the translation (you may adjust the scale factor as needed)
    extrinsic[:3, 3] += translation
    # Update the parameters with the modified extrinsic matrix
    params.extrinsic = extrinsic
    view_ctl.convert_from_pinhole_camera_parameters(params)
    return False  # Returning False allows further event processing
...
def register_camera_key_callbacks(vis):
    # Adjust the step size as necessary
    step = 0.1
    vis.register_key_callback(ord("W"), lambda vis: translate_camera(vis, dz=-step))
    vis.register_key_callback(ord("S"), lambda vis: translate_camera(vis, dz=step))
    vis.register_key_callback(ord("A"), lambda vis: translate_camera(vis, dx=step))
    vis.register_key_callback(ord("D"), lambda vis: translate_camera(vis, dx=-step))
    vis.register_key_callback(ord("Q"), lambda vis: translate_camera(vis, dy=step))
    vis.register_key_callback(ord("E"), lambda vis: translate_camera(vis, dy=-step))
    vis.register_key_callback(256, lambda vis: quit_visualizer(vis))
...
def visualize(scene_path, cfg):
    ...
    vis.create_window(width=int(cfg['viz_w'] * cfg['view_scale']),
                      ...
                      visible=True)
    register_camera_key_callbacks(vis)
    ...
    while True:
        ...
        if os.path.exists(scene_path + str(idx) + ".npz"):
            try:
                # check if file is in use
                os.rename(scene_path + str(idx) + ".npz", scene_path + str(idx) + ".npz")
                # render next step if not
                data_ready = True # for first step
            except:
                pass
        ...
        if data_ready:
            ...
            print("Current Frame: " + str(idx))
            if started:
                current_params = view_control.convert_to_pinhole_camera_parameters()

            w2c, k = load_camera(cfg, scene_path + str(idx) + ".npz")
            scene_data, scene_depth_data, all_w2cs = load_scene_data(scene_path + str(idx) + ".npz", w2c, k)
            ...
            render_options.background_color = np.array([0.0, 0.0, 0.0])
            render_options.point_size = cfg['view_scale']
            render_options.light_on = False
            if started:
                view_control.convert_from_pinhole_camera_parameters(current_params, allow_arbitrary=True)
            ...
            print(f"Time to draw frame: {elapsed*1000:.2f} ms")
...
if __name__ == "__main__":
    ...
    if "scene_path" not in experiment.config:
        results_dir = os.path.join(
            experiment.config["workdir"], "checkpoints"
        )
        scene_path = os.path.join(results_dir, "params")
    else:
        scene_path = experiment.config["scene_path"]
    viz_cfg = experiment.config["viz"]

    if experiment.config['use_overlay']:
