...
async def display_video(config, track: VideoStreamTrack):
    ...
    # config['tracking']['forward_prop'] = False
    
    max_n_frames = 100_000

    # while True:
    progress_bar = tqdm(list(range(max_n_frames)), desc="Processing frames")
    for time_idx in progress_bar:
        ...
        H, W, C = img_bgr.shape
        W = W // 2 - 1
        depth_bgr, color_bgr = img_bgr[:, :W], img_bgr[:, W+2:]
        ...
        if first_frame:
            initial_c2w = torch.eye(4, dtype=torch.float32, device='cuda')
            initial_c2w[0, 0] = -1
            initial_c2w[1, 1] = -1
            initial_c2w[2, 2] = 1
        ...
        #     break
        progress_bar.set_postfix_str(f'num_gaussians: {params["means3D"].shape[0]}')
        viewer.lock.release()
