...
template <typename T>
__global__ void fully_fused_projection_packed_iso_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_projection_packed_iso_fwd_tensor(
    ...
    if (C && N) {
        ...
        fully_fused_projection_packed_iso_fwd_kernel<float>
